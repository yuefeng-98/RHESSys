...
RUN Rscript -e "install.packages(c('chron','dplyr','tidyr','readr','purrr','data.table','lubridate','sensitivity','hydroGOF','parallel','randtoolbox','rlang','tools','yaml','usethis','httr','roxygen2','rversions','devtools'))"
RUN Rscript -e "library(devtools)" -e "setwd('../RHESSysIOinR/')" -e "build()" -e "install()" -e "library(RHESSysIOinR)" 
